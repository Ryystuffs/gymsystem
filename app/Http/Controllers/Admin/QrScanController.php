...
class QrScanController extends Controller
{
    ...
    public function handle(User $user)
    {
        ...
        if (!UserMemberships::where('user_id', $user->id)->where('is_active', true)->exists()) {
            ...
            if (MemberSessions::where('user_membership_id', $membership->id)->whereNull('check_out')->exists()) {
                $session = MemberSessions::where('user_membership_id', $membership->id)->whereNull('check_out')->first();
                $session->check_out = now();
                $session->save();
                return response()->json(['message' => "User ID: {$user->id} checked out at {$session->check_out}"]);
            }
        }
        return response()->json(['message' => "QR code scanned for user sessions."]);
    }
}
